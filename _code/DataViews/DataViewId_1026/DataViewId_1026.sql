/******************************************************************************************************************************************************
    ...
    This SQL is found in DataViewId:1026
    Currently, this DataView is named "LG NextLeaders, Yes RDT"
    
    References:
    - This SQL is used to sync GroupMembers of GroupId: 131749
    
    Path:
    _code/DataViews/DataViewId_1026/DataViewId_1026.sql
...
DECLARE @GroupTypeRole_NextLeader TABLE (
    [Id] int NOT NULL
);
...
INSERT INTO @GroupTypeRole_NextLeader ([Id])
SELECT
    [Id]
FROM
    [GroupTypeRole]
WHERE
    [GroupTypeId] IN (SELECT [Id] FROM @GroupType_LifeGroup)
    AND
    [Id] = 175 --This is the "NextLeader" Role
;
...
WHERE
    ...
    AND
    gm.[GroupRoleId] IN (SELECT [Id] FROM @GroupTypeRole_NextLeader)
    AND
